...
- A Talos Enterprise Linux entitlement (either an Omni subscription or a standalone TEL support contract) with an organization provisioned on Enterprise Image Factory. If you cannot log in, contact [support](https://www.siderolabs.com/support).
...
The Enterprise Image Factory login screen asks for your work email and routes you to your organization's identity provider. You need to log in to create an API token in the next step.
...
The factory returns a 64-character hex ID. This ID, together with a Talos version and architecture, uniquely identifies a reproducible image.
...
Enterprise Image Factory attaches SLSA Provenance v1 and SPDX SBOM attestations to every installer OCI image. Use `cosign` to verify them against the signing identity from Siderolabs' GitHub Actions workflows:

```bash
cosign verify-attestation \
  --certificate-identity-regexp 'https://github.com/siderolabs/' \
  --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
  --registry-username "$EIF_ORG" \
  --registry-password "$EIF_TOKEN" \
  factory.siderolabs.com/installer/$SCHEMATIC:$TALOS_VERSION
```

A successful run prints the attestation payload to stdout. For verifying detached `.sigstore.json` bundles for ISOs and disk images, see [Verify Signatures and Attestations](./verify-signatures-and-attestations).

### Step 7: Boot your machine and create a cluster
...
Clusters switch to enterprise images on their next upgrade. Any change that triggers a schematic rebuild will do: bumping the Talos version, adding or removing a system extension, or changing a kernel argument. On that upgrade, Omni checks whether an enterprise build exists for the target version (available for Talos 1.13 and later) and pulls it automatically. Until a cluster is upgraded, its machines continue running community Talos.

To get started with Omni, follow the [Getting Started with Omni](../../omni/getting-started/getting-started) guide. To move machines that are already running community Talos to Talos Enterprise Linux, see [Migrate Clusters to TEL](./migrate-clusters-to-tel).
