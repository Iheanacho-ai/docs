...
kubectl create secret -n synology-csi generic client-info-secret \
  --from-file=config/client-info.yml
```
...
## Configure node.yaml for Talos
 
The official `synology-csi` driver supports Talos Linux as of v1.2.1.
You do not need to build a custom image.
Use `synologyopensource/synology-csi:v1.2.1` (or `latest`) directly.
 
To run correctly on Talos, you must add two arguments to the node plugin container in `deploy/kubernetes/v1.20/node.yml`:
...
These arguments tell the driver where to find the host filesystem and the `iscsiadm` binary that Talos provides via the `iscsi-tools` extension.
Without them, the driver will fail with `chroot: can't execute '/usr/bin/env': No such file or directory`.
