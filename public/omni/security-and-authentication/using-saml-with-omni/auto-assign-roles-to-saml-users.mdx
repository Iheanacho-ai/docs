...
When SAML authentication is enabled, Omni can automatically assign roles to new users based on their SAML attributes. To configure this, use SAMLLabelRule resources to map SAML attribute labels to Omni roles. If no rules are configured, [roles must be assigned manually](./add-a-user-to-omni-with-saml-enabled) by an administrator.
