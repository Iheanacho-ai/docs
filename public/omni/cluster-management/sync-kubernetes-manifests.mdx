...
<Note>Manifests are not visible or manageable from the Omni UI at this time.</Note>
...
To inspect the sync status of manifests for a cluster, use `omnictl` to get the `ClusterKubernetesManifestsStatus` resource:
...
If a manifest cannot be applied due to a configuration error (for example, a typo in a field name or an invalid API version), the error is reported in the status.
...
- The Omni UI does not display or allow management of synced manifests.
