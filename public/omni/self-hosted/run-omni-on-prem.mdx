...
<Note>If you plan to use an external etcd cluster instead of Omni's embedded etcd, also open port **2379** (TCP) and restrict access to your Omni host's IP only. See [Run an etcd Cluster On-Prem](./run-an-etcd-cluster-on-prem) for setup instructions.</Note>
...
### 6.1: Install htpasswd

`htpasswd` is used to generate a bcrypt password hash for the Dex static user.

```bash
sudo apt-get install -y apache2-utils
```

### 6.2: Create a password hash for the admin user
...
### 6.3: Write the Dex configuration
...
Use this if you have a running external etcd cluster. See [Run an etcd Cluster On-Prem](./run-an-etcd-cluster-on-prem) to set one up first.

Omni authenticates to etcd using a client certificate. Generate one now using the `client` signing profile created in Step **4.2**:
...
Start Omni with the external etcd flags. The `--etcd-embedded=false` flag disables the internal etcd instance and `--etcd-endpoints` points Omni to your cluster:
...
docker run -d \
  ...
  -v $(pwd)/server-chain.pem:/server-chain.pem:ro,Z \
  -v $(pwd)/omni.asc:/omni.asc:ro,Z \
  -v $(pwd)/ca.pem:/etcd/ca.crt:ro,Z \
  -v $(pwd)/client.pem:/etcd/client.crt:ro,Z \
  -v $(pwd)/client-key.pem:/etcd/client.key:ro,Z \
  -v $HOME/sqlite:/_out/sqlite:rw,Z \
  ghcr.io/siderolabs/omni:${OMNI_VERSION} \
    --name=onprem-omni \
    --cert=/server-chain.pem \
...
Clear your local DNS cache to ensure the new hostname mappings take effect immediately.
