...
See [Run Omni Options](./omni-deployment-options) for more context on when each mode is appropriate.
...
|------|----------|---------|
| 2379 | TCP | etcd client API (restrict to Omni host IP only) |
| 2380 | TCP | etcd peer communication (restrict to etcd node IPs only) |

## Before you begin

Throughout this guide, the three etcd nodes are referred to as:

| Node | Variable | Example IP |
|------|----------|------------|
| Node 1 | `ETCD1_IP` | `10.0.0.1` |
| Node 2 | `ETCD2_IP` | `10.0.0.2` |
| Node 3 | `ETCD3_IP` | `10.0.0.3` |

Run all commands in **Steps 1–4** on **Node 1** only. Steps 5 and 6 are run on all three nodes.
...
```bash
export ETCD1_IP=10.0.0.1
export ETCD2_IP=10.0.0.2
export ETCD3_IP=10.0.0.3
export OMNI_IP=10.0.0.10   # IP of the host running Omni
```
...
Each node gets its own certificate with its IP address embedded as a SAN. etcd uses these IPs to verify the identity of incoming peer and client connections. Run all three commands on Node 1.
...
## Step 4: Distribute certificates (Node 1 only)

Each node needs its own certificate and key, plus the shared CA certificate. The Omni client certificate goes to the Omni host, where it will be mounted into the Omni container.

**To Node 2:**

```bash
scp ca.pem etcd2.pem etcd2-key.pem ubuntu@${ETCD2_IP}:~/
```

**To Node 3:**

```bash
scp ca.pem etcd3.pem etcd3-key.pem ubuntu@${ETCD3_IP}:~/
```

**To the Omni host:**

```bash
scp ca.pem client.pem client-key.pem ubuntu@${OMNI_IP}:~/etcd-certs/
```
...
## Step 8: Connect Omni to the etcd cluster
