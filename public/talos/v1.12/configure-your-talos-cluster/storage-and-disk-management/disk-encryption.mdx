...
Starting from Talos v1.12, you can choose which Platform Configuration Register (PCR)
values to bind to when using TPM-based disk encryption.

This allows more flexibility across different hardware and Secure Boot setups.

Use the `options.pcrs` field under the `tpm` key in your encryption configuration.
If no PCRs are specified, Talos defaults to **PCR 7** to stay compatible with older versions.
...
> **Note**: Signed PCR policies are still bound to PCR 11. This flexibility helps avoid issues caused by varying PCR 7 states due to Secure Boot differences.